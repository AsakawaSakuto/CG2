...
void ParticleDescriptorAllocator::Initialize(DirectXCommon* dx, uint32_t begin, uint32_t end) {
    dx_ = dx;
    begin_ = begin;
    end_ = end;
...
uint32_t ParticleDescriptorAllocator::Allocate() {
    // // 返却済みがあれば再利用（穴埋め）
    if (!free_.empty()) {
...
void ParticleDescriptorAllocator::Free(uint32_t index) {
    // // レンジ内だけ返却許可
