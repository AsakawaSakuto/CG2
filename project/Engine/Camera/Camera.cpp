...
void Camera::UpdateFrustum() {
	...
	float leftDistance = viewProjection.m[3][3] + viewProjection.m[3][0];
	...
	float rightDistance = viewProjection.m[3][3] - viewProjection.m[3][0];
	...
	float bottomDistance = viewProjection.m[3][3] + viewProjection.m[3][1];
	...
	float topDistance = viewProjection.m[3][3] - viewProjection.m[3][1];
	...
	float nearDistance = viewProjection.m[3][3] + viewProjection.m[3][2];
	...
	float farDistance = viewProjection.m[3][3] - viewProjection.m[3][2];
	
	float distances[6] = { leftDistance, rightDistance, bottomDistance, topDistance, nearDistance, farDistance };
	...
	for (int i = 0; i < 6; i++) {
		...
		if (length > 0.0f) {
			...
			float normalizedDistance = distances[i] / length;
			
			// distanceからcenterを計算（平面上の原点に最も近い点）
			frustum_.planes[i].center.x = frustum_.planes[i].normal.x * normalizedDistance;
			frustum_.planes[i].center.y = frustum_.planes[i].normal.y * normalizedDistance;
			frustum_.planes[i].center.z = frustum_.planes[i].normal.z * normalizedDistance;
		}
	}
}
...
bool Camera::IsInFrustum(const Vector3& position, float radius) const {
	...
	for (int i = 0; i < 6; i++) {
		// 点から平面までの符号付き距離を計算
		// d = dot(normal, point - center)
		float distance = 
			frustum_.planes[i].normal.x * (position.x - frustum_.planes[i].center.x) +
			frustum_.planes[i].normal.y * (position.y - frustum_.planes[i].center.y) +
			frustum_.planes[i].normal.z * (position.z - frustum_.planes[i].center.z);
